    ...
    if args.footnotes:
        ...
        for fn_id, text in sorted(fns.items()):
            ...
            if idx != -1:
                ...
                if end < len(text):
                    snippet = snippet + "..."
                print(f"[Footnote {fn_id}] {_show_nbsp(snippet)}")
...
def cmd_footnotes(args):
    ...
    if args.id is not None:
        if args.id in fns:
            print(f"[{args.id}] {_show_nbsp(fns[args.id])}")
        else:
            print(f"Footnote {args.id} not found.", file=sys.stderr)
            sys.exit(1)
    else:
        for fn_id, text in sorted(fns.items()):
            print(f"[{fn_id}] {_show_nbsp(text)}")
...
def cmd_comments(args):
    ...
    if args.json:
        ...
        for c in comments:
            print(f"[{c['id']}] {c['author']} ({c['date']}): {_show_nbsp(c['text'])}")
...
def cmd_changes(args):
    ...
    for c in changes:
        print(f"[{c['type']}] {c['author']}: \"{_show_nbsp(c['text'])}\"")
...
def cmd_replace(args):
    ...
    old = _parse_nbsp(args.old)
    new = _parse_nbsp(args.new or "")
    context = _parse_nbsp(args.context) if args.context else None
